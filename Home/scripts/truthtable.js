...
{
    if(event.which==13) evaluateMe();
    if(event.keyCode==13) evaluateMe();
}
...
function evaluateMe()
{
	...
	k=0;
	s='<TABLE border=1 cellspacing=2 bordercolor="#ffffff" bgcolor="#ffffff"><TR>';
	for(i=0;i<vars.length;++i) s+='<TD bordercolor="#ffffff"><B>'+vars[i]+'</TD>';
	s+='<TD bordercolor="#ffffff"><B>output</TD></TR>';

	for(i=0;i<nosteps;++i)
	{
		// format output
		s+'<TR>';
		for(j=0;j<vars.length;++j) s+='<TD>'+values[j]+'</TD>';
		s+='<TD ALIGN=CENTER>';
